...
const out=await fs.mkdtemp(path.join(here,'outputs','depot-visual-native-'));
...
try{
 ...
 for(const size of ['small','standard','large','massive']){
  ...
  const screenshots=[];
  for(const team of [1,2])for(const view of ['overview','repair','yard','refuel']){
   if(view==='overview')await click('Team '+team+' overview');
   else{
    await evaluate(`(()=>{const s=document.querySelector('[aria-label="Authored preview building"]');const o=[...s.options].find(o=>o.textContent.includes('Team '+${team}+' · ')&&o.textContent.toLowerCase().includes(${JSON.stringify(view==='yard'?'power cell':view)}));if(!o)throw new Error('Missing service pad');s.value=o.value;s.dispatchEvent(new Event('change',{bubbles:true}));})()`);
    await click('Close preview building');
   }
   if(view==='yard'){
    const rect=await evaluate(`(()=>{const r=document.querySelector('canvas').getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2};})()`);
    for(let step=0;step<20;step++)await send('Input.dispatchMouseEvent',{type:'mouseWheel',...rect,deltaX:0,deltaY:100});
   }
   await new Promise(r=>setTimeout(r,1200));
   const file=path.join(out,size+'-team'+team+'-'+view+'.png');await fs.writeFile(file,Buffer.from((await send('Page.captureScreenshot',{format:'png'})).data,'base64'));screenshots.push({team,view,path:file,sha256:await hash(file)});
  }
