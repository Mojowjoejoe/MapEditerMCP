...
const out=await fs.mkdtemp(path.join(here,'outputs','service-focus-native-'));
...
try{
 ...
 await click('Focus draft path','.service-route-panel');
 ...
 report.focusCamera=true;report.draftOverlay={showAndCloseCaptured:true,modeSwitchCaptured:true,mapHistoryUnchanged:true};
