...
const out=await fs.mkdtemp(path.resolve('outputs/maintenance-library-restart-'));
...
try{
 ...
 const {createBlankProject}=await import('../../../lib/wulfram.ts');const destination=createBlankProject('Restart Maintenance destination',129);destination.terrain.worldWidth=24000;destination.terrain.worldHeight=20000;destination.terrain.heights.fill(0);const destinationFile=path.join(out,'destination.json');await fs.writeFile(destinationFile,JSON.stringify(destination));
 ...
 const shot=await send('Page.captureScreenshot',{format:'png'});const screenshot=path.join(out,'maintenance-library-restart.png');await fs.writeFile(screenshot,Buffer.from(shot.data,'base64'));
 report.artifacts=await Promise.all(['destination.json','placed.json','persisted-library.json','maintenance-library-restart.png'].map(async name=>({path:path.join(out,name),sha256:await hash(path.join(out,name))})));
