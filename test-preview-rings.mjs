...
const out=await fs.mkdtemp(path.join(here,'outputs','preview-rings-native-'));
...
try{
 ...
 for(const size of ['small']){
  ...
  const screenshots=[];
  await evaluate(`(()=>{const s=document.querySelector('[aria-label="Authored preview building"]');const o=[...s.options].find(o=>o.textContent.includes('Team 1 · Repair'));if(!o)throw new Error('Missing repair');s.value=o.value;s.dispatchEvent(new Event('change',{bubbles:true}));})()`);
  await click('Close preview building');await new Promise(r=>setTimeout(r,1200));
  const focused=await snapshot();
  for(const [view,boundaries,overlays] of [['boundaries-off',false,true],['rings-on',true,true],['master-off',true,false],['restored',true,true]]){
   await setCheck('Building area circles',boundaries);await setCheck('Show display overlays',overlays);
   await new Promise(r=>setTimeout(r,400));assert.deepEqual(await snapshot(),focused);
   const file=path.join(out,view+'.png');await fs.writeFile(file,Buffer.from((await send('Page.captureScreenshot',{format:'png'})).data,'base64'));screenshots.push({view,path:file,sha256:await hash(file)});
  }
  assert.notEqual(screenshots[0].sha256,screenshots[1].sha256);
