...
const out=await fs.mkdtemp(path.join(here,'outputs','service-point-pick-native-'));
...
try{
 ...
 const pointValues=()=>evaluate(`['X','Y'].map(axis=>Number(document.querySelector('[aria-label="Path point 1 '+axis+'"]').value))`);
 const allPointValues=()=>evaluate(`Array.from(document.querySelectorAll('.service-route-panel fieldset'),field=>['X','Y'].map(axis=>Number(field.querySelector('input[aria-label$=" '+axis+'"]').value)))`);
 const originalPoint=await pointValues(),originalPoints=await allPointValues();
 await click('Pick point 1 on terrain','.service-route-panel');
 assert.ok(await evaluate(`document.body.textContent.includes('Pick service-path point 1')`));
 assert.equal(await evaluate(`[...document.querySelectorAll('.service-route-panel button')].some(b=>b.textContent.startsWith('Pick point 2 on terrain'))`),false);
 await send('Input.dispatchKeyEvent',{type:'keyDown',key:'Escape',code:'Escape',windowsVirtualKeyCode:27});
 await send('Input.dispatchKeyEvent',{type:'keyUp',key:'Escape',code:'Escape',windowsVirtualKeyCode:27});
 await probe(()=>evaluate(`!document.body.textContent.includes('Pick service-path point 1')`),'Escape cancels pick');
 assert.deepEqual(await pointValues(),originalPoint);assert.deepEqual(await snapshot(),overlayBefore);
 await click('Pick point 1 on terrain','.service-route-panel');
 await probe(()=>evaluate(`document.body.textContent.includes('Pick service-path point 1')`),'terrain pick armed');
 await evaluate('new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)))');
 const center=await evaluate(`(()=>{const r=document.querySelector('canvas').getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2};})()`);
 ...
 await probe(()=>evaluate(`!document.body.textContent.includes('Pick service-path point 1')`),'terrain consumes pick');
 const picked=await pointValues();assert.deepEqual((await allPointValues()).slice(1),originalPoints.slice(1));assert.ok(picked.every(Number.isFinite));assert.notDeepEqual(picked,originalPoint);
 assert.ok(picked[0]>=0&&picked[0]<=fixture.terrain.worldWidth&&picked[1]>=0&&picked[1]<=fixture.terrain.worldHeight);
 assert.deepEqual(await snapshot(),overlayBefore);
 assert.equal(await evaluate(`[...document.querySelectorAll('.service-route-panel button')].find(b=>b.textContent==='Apply path changes').disabled`),true);
 await fs.writeFile(path.join(out,'point-picked.png'),Buffer.from((await send('Page.captureScreenshot',{format:'png'})).data,'base64'));
 await click('Load current path','.service-route-panel');assert.deepEqual(await pointValues(),originalPoint);
 await click('Insert after point 1','.service-route-panel');
 await click('Pick point 2 on terrain','.service-route-panel');
 await probe(()=>evaluate(`document.body.textContent.includes('Pick service-path point 2')`),'middle pick armed');
 await evaluate('new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)))');
 await send('Input.dispatchMouseEvent',{type:'mousePressed',...center,button:'left',clickCount:1});
 await send('Input.dispatchMouseEvent',{type:'mouseReleased',...center,button:'left',clickCount:1});
 await probe(()=>evaluate(`!document.body.textContent.includes('Pick service-path point 2')`),'middle pick completed');
 const pickedPath=await allPointValues();assert.equal(pickedPath.length,3);
 assert.deepEqual(pickedPath[0],originalPoints[0]);assert.deepEqual(pickedPath.at(-1),originalPoints.at(-1));
 assert.deepEqual(await snapshot(),overlayBefore);
 await click('Preview path changes','.service-route-panel');
 assert.ok(await evaluate(`document.querySelector('.service-route-panel').textContent.includes('Access checks passed')`));
 await click('Apply path changes','.service-route-panel');
 const pickedApplied=await snapshot();assert.equal(pickedApplied.undoCount,overlayBefore.undoCount+1);
 assert.deepEqual((await call('inspect_service_routes',{sessionId})).routes.find(r=>r.padId===movedPad.id).points,pickedPath);
 await click('Undo','.history-controls');assert.deepEqual((await snapshot()).project,overlayBefore.project);
 await click('Load current path','.service-route-panel');
 // Undo changes revision/history; use the new full snapshot for subsequent cancellation checks.
 Object.assign(overlayBefore,await snapshot());
 report.pickedPathApplyUndo=true;
 await click('Pick point 1 on terrain','.service-route-panel');
 await evaluate(`document.querySelector('.service-route-panel details').open=false`);
 await probe(()=>evaluate(`!document.body.textContent.includes('Pick service-path point 1')`),'close cancels pick');
 await evaluate(`document.querySelector('.service-route-panel details').open=true`);
 await click('Pick point 1 on terrain','.service-route-panel');await click('Terrain');
 await probe(()=>evaluate(`!document.body.textContent.includes('Pick service-path point 1')`),'mode cancels pick');
 await click('Base builder');await click('Rules');
 await probe(()=>evaluate(`!!document.querySelector('.service-route-panel')`),'return to rules');
 assert.deepEqual(await snapshot(),overlayBefore);
 report.pointPicking={terrainClickChangesDraftOnly:true,escapePreservesDraft:true,closeCancels:true,modeCancels:true,padEndpointExcluded:true,applyRequiresPreview:true};
 await evaluate(`(()=>{document.querySelector('.service-route-panel').open=true;document.querySelector('.service-route-panel details').open=true;})()`);
 await probe(()=>evaluate(`[...document.querySelectorAll('.service-route-panel button')].find(b=>b.textContent==='Pick point 1 on terrain').getClientRects().length>0`),'pick control visible after mode return');
 for(const dialog of ['library','game']){
  await click('Pick point 1 on terrain','.service-route-panel');
  await probe(()=>evaluate(`document.body.textContent.includes('Pick service-path point 1')&&[...document.querySelectorAll('.service-route-panel button')].find(b=>b.textContent==='Pick point 1 on terrain').getAttribute('aria-pressed')==='true'`),'armed pick before dialog');
  if(dialog==='library')await click('Browse base library');else{await evaluate(`[...document.querySelectorAll('.editor-menu-bar summary')].find(s=>s.textContent==='Tools').click()`);await click('Test in game…');}
  await probe(()=>evaluate(`!document.body.textContent.includes('Pick service-path point 1')`),'dialog cancels pick');
  if(dialog==='library')await click('Close library');else{
   await send('Input.dispatchKeyEvent',{type:'keyDown',key:'Escape',code:'Escape',windowsVirtualKeyCode:27});
   await send('Input.dispatchKeyEvent',{type:'keyUp',key:'Escape',code:'Escape',windowsVirtualKeyCode:27});
  }
  await probe(()=>evaluate(`!document.querySelector('[role="dialog"]')`),'dialog dismissed');
  assert.equal(await evaluate(`[...document.querySelectorAll('.service-route-panel button')].find(b=>b.textContent==='Pick point 1 on terrain').getAttribute('aria-pressed')`),'false');
  assert.deepEqual(await snapshot(),overlayBefore);
 }
 report.pointPicking.dialogsCancelWithoutRevival=true;
 await click('Pick point 1 on terrain','.service-route-panel');
 await probe(()=>evaluate(`document.body.textContent.includes('Pick service-path point 1')`),'armed before import');
 await send('DOM.setFileInputFiles',{nodeId:input.nodeId,files:[]});
 await send('DOM.setFileInputFiles',{nodeId:input.nodeId,files:[fixturePath]});
 await probe(async()=>JSON.stringify((await snapshot()).project)===JSON.stringify(fixture),'replacement map imported');
 await probe(()=>evaluate(`!document.body.textContent.includes('Pick service-path point 1')`),'source replacement cancels pick');
 const importedSnapshot=await snapshot();
 await click('Load current path','.service-route-panel');
 assert.equal(await evaluate(`[...document.querySelectorAll('.service-route-panel button')].find(b=>b.textContent==='Pick point 1 on terrain').getAttribute('aria-pressed')`),'false');
 assert.deepEqual(await snapshot(),importedSnapshot);
 report.pointPicking.mapChangeCancels=true;
 Object.assign(overlayBefore,importedSnapshot);
