...
const out=await fs.mkdtemp(path.join(here,'outputs','maintenance-portable-native-'));
const fixture=JSON.parse(await fs.readFile(path.join(root,'outputs/maintenance-pocket-portable-native.json'),'utf8'));
...
try{
 ...
 const sourceRoutes=JSON.parse(fixture.baseLayouts.find(l=>l.id===fixture.activeBaseLayoutId).metadata['forge.serviceRoutes.v1']).routes;
 ...
 await evaluate(`(()=>{document.querySelector('.authored-library-panel').open=true;const input=document.querySelector('[aria-label="Authored library name"]');Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set.call(input,'Maintenance service paths');input.dispatchEvent(new Event('input',{bubbles:true}));})()`);
 ...
 const destination=createBlankProject('Maintenance cross-map destination',129);destination.terrain.worldWidth=24000;destination.terrain.worldHeight=20000;destination.terrain.heights.fill(0);
 const destinationPath=path.join(out,'destination.json');await fs.writeFile(destinationPath,JSON.stringify(destination));
 const targetDom=await send('DOM.getDocument'),targetInput=await send('DOM.querySelector',{nodeId:targetDom.root.nodeId,selector:'input[type="file"][multiple]'});
 await send('DOM.setFileInputFiles',{nodeId:targetInput.nodeId,files:[destinationPath]});
 await probe(async()=>{const s=await snapshot();return s.project.name===destination.name;},'different destination map');
 const destinationBefore=await snapshot(),crossRequest={activeLayoutId:destinationBefore.project.activeBaseLayoutId,layoutId:'cross-map-pocket',frame:{origin:[12000,10000,0],yaw:Math.PI/6},terrainMode:'conform'};
 await call('place_authored_base',{sessionId,expectedRevision:destinationBefore.revision,previewOnly:true,packageJson:capture.packageJson,authoredRequest:crossRequest});assert.deepEqual(await snapshot(),destinationBefore);
 await call('place_authored_base',{sessionId,expectedRevision:destinationBefore.revision,previewOnly:false,packageJson:capture.packageJson,authoredRequest:crossRequest});
 const cross=await snapshot(),crossLayout=cross.project.baseLayouts.find(l=>l.id===cross.project.activeBaseLayoutId);
 assert.equal(cross.project.entities.length,40);assert.deepEqual(cross.project.terrain,destinationBefore.project.terrain);
 const crossRoutes=JSON.parse(crossLayout.metadata['forge.serviceRoutes.v1']).routes;assert.equal(crossRoutes.length,8);
 for(const r of crossRoutes){const pad=crossLayout.entities.find(e=>e.id===r.padId);assert.ok(pad);assert.ok(Math.hypot(r.points.at(-1)[0]-pad.position[0],r.points.at(-1)[1]-pad.position[1])<1e-5);}
 await call('undo',{sessionId,expectedRevision:cross.revision});assert.deepEqual((await snapshot()).project,destinationBefore.project);
 report.checks={exactImport:true,mcpCaptureV3:true,previewUnchanged:true,staleRejected:true,rotatedPlacement:true,padIdsRemapped:true,recapture:true,guiCapture:true,guiLibrarySaveReload:true,guiPreviewApplyUndo:true,priorLayoutTerrainPreserved:true,crossMapPreviewApplyUndo:true};report.passed=true;
