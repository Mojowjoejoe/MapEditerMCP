...
const out=await fs.mkdtemp(path.join(here,'outputs','depot-exchange-native-'));
...
try{
 ...
 for(const size of ['small','standard','large','massive']){
  ...
  const args={sessionId,expectedRevision:sourceState.revision},initial=await call('inspect_authored_library',args);
  const entry={id:'exchange-'+size,name:'Depot '+size,base:pack};
  ...
  const exported=(await call('inspect_authored_library',args)).library;
  assert.deepEqual(exported.entries.at(-1),entry);
  const exportFile=path.join(out,'library-'+size+'.json');await fs.writeFile(exportFile,JSON.stringify(exported));
  const exchange=JSON.parse(await fs.readFile(exportFile,'utf8'));assert.deepEqual(exchange,exported);
  const removed=await call('edit_authored_library',{...args,expectedLibraryRaw:stored.raw,libraryEditJson:JSON.stringify({operation:'remove',id:entry.id})});
  const absent=await call('inspect_authored_library',args);assert.equal(absent.raw,removed.raw);assert.deepEqual(absent.library.entries,initial.library.entries);assert.ok(!absent.library.entries.some(e=>e.id===entry.id));
  const edit={operation:'import',library:exchange};
  const proposed=await call('edit_authored_library',{...args,expectedLibraryRaw:removed.raw,previewOnly:true,libraryEditJson:JSON.stringify(edit)});
  assert.deepEqual(proposed.library.entries,exported.entries);assert.equal((await call('inspect_authored_library',args)).raw,removed.raw);
  const imported=await call('edit_authored_library',{...args,expectedLibraryRaw:removed.raw,libraryEditJson:JSON.stringify(edit)});
  assert.deepEqual(imported.after.entries,exported.entries);assert.deepEqual(await snapshot(),sourceState);
  const actual=await call('inspect_authored_library',args);assert.equal(actual.raw,imported.raw);assert.deepEqual(actual.library.entries,exported.entries);
  captured.packageJson=JSON.stringify(actual.library.entries.find(e=>e.id===entry.id).base);
  const libraryArtifact={path:exportFile,sha256:await hash(exportFile)};
  ...
  report.cases.push({size,libraryArtifact,libraryExchange:true,recaptured:{path:recapturePath,sha256:await hash(recapturePath)},source:sourceArtifact,destination:destinationArtifact,placed:{path:placedPath,sha256:await hash(placedPath)},units:layout.entities.length,routes:routes.length,independentCoordinates:true,previewUnchanged:true,priorLayoutTerrainPreserved:true,recapture:true,undo:true});
 }
 const finalState=await snapshot();const finalLibrary=await call('inspect_authored_library',{sessionId,expectedRevision:finalState.revision});const libraryPath=path.join(out,'complete-library.json');await fs.writeFile(libraryPath,JSON.stringify(finalLibrary.library));report.libraryArtifact={path:libraryPath,sha256:await hash(libraryPath)};
